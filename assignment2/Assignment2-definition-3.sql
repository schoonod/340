...
-- For part one of this assignment you are to make a database with the following specifications and 
-- run the following queries
-- Table creation queries should immedatley follow the drop table queries, this is to facilitate 
-- testing on my end
...
CREATE TABLE `client` (
	...
	`dob` date,
	PRIMARY KEY (`id`),
	UNIQUE KEY client_unique (first_name, last_name)
) ENGINE=InnoDB;
------------------------------------------------------------------------------------------------------
...
CREATE TABLE `employee` (
	...
	`date_joined` date,
	PRIMARY KEY (`id`),
	UNIQUE KEY employee_unique (first_name, last_name)
) ENGINE=InnoDB;
------------------------------------------------------------------------------------------------------
...
CREATE TABLE `project` (
	...
	UNIQUE KEY project_unique (name)
) ENGINE=InnoDB;
------------------------------------------------------------------------------------------------------
-- Create a table called works_on with the following properties, this is a 
-- table representing a many-to-many relationship between employees and projects:
...
INSERT INTO employee (first_name,last_name,date_joined) VALUES 
("Adam","Lowd","2009-1-1"),
("Michael", "Fern", "2013-6-5"),
("Deena", "Young", "2013-11-10");
------------------------------------------------------------------------------------------------------
-- insert the following project instances into the project table 
-- (you should use a subquery to set up foriegn key referecnes, no hard coded numbers):
...
((SELECT id FROM employee WHERE first_name="Adam" AND last_name="Lowd"),(SELECT id FROM project WHERE name="Diamond"),"2012-1-1");
